...
# local files
from scattering_ac_cross import Scattering
from scattering_ac_cross import Ef,Wf,U,a0
...
def main():
    aN=18
    absorb_ae=np.zeros(aN,dtype=np.float64)
    material_num_new=sys.argv[1]
    for i in range(aN):
        mypool=Pool(processes=n_pool)
        input1=(5*i*np.ones(n_pool).astype(np.int64)).tolist()
        #input1=(0*np.ones(n_pool).astype(np.int64)).tolist()
        result=mypool.map_async(dec_MC,input1)
        mypool.close()
        mypool.join()
        absorb_c=sum(result.get())/n_pool
        absorb_ae[i]=absorb_c
        print('incident angle ',5*i,' result with',N_electron*n_pool,'electrons:',absorb_c)
    print(absorb_ae)
    np.savetxt('angle_scan_of_material_'+material_num_new+'.txt',absorb_ae)
    pass
